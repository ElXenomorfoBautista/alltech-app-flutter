...
class WorkOrderProvider extends BaseProvider {
  Future<Response> getWorkOrdersUser({Map<String, dynamic>? filter}) async {
    try {
      ...
      return await get('/work-orders/user?filter=$filterString');
    } catch (e) {
  ...
  Future<Response> getWorkOrderById(int id) async {
    try {
      ...
      final filterString = Uri.encodeComponent(jsonEncode(defaultFilter));

      return await get('/work-orders/$id?filter=$filterString');
    } catch (e) {
      return handleError(e);
    }
  }
